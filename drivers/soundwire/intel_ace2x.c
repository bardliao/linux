				       ...
				       struct sdw_bpt_msg *msg)
{
	...
	pr_err("bard: rx alignment %d tx alignment %d pdi1_buffer_size was %d pdi0_buffer_size was %d\n",
		rx_alignment, tx_alignment, pdi1_buffer_size, pdi0_buffer_size);
	...
	if (command) { /* read */
		...
		pr_err("bard: pdi0_buffer_size %d tx_pad %d fake_size %d\n",
			pdi0_buffer_size, tx_pad, fake_size);
	} else { /* write */
		/*
