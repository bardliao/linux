	...
	for (l = 0; l < SDW_MAX_LANES; l++) {
		...
		for (i = 0; i < count; i++) {
			...
			list_for_each_entry(m_rt, &bus->m_rt_list, bus_node) {
				dev_info(bus->dev, "bard: %s m_rt %p\n", __func__, m_rt);
...
static bool is_all_peripherals_connected_to(struct sdw_master_runtime *m_rt, unsigned int lane)
{
	...
	list_for_each_entry(s_rt, &m_rt->slave_rt_list, m_rt_node) {
		...
		for (i = 1; i < SDW_MAX_LANES; i++) {
			if (slave_prop->lane_maps[i] == lane) {
				dev_info(&s_rt->slave->dev, "lane_maps[%d] is connected to %d\n", i, slave_prop->lane_maps[i]);
				break;
			}
		}
		if (i == SDW_MAX_LANES) {
...
multilane:
		...
		for (i = 1; i < SDW_MAX_LANES; i++) {
			...
			if (required_bandwidth <= curr_dr_freq - bus->lane_used_bandwidth[i]) {
				...
				if (!is_all_peripherals_connected_to(m_rt, slave_prop->lane_maps[i])) {
					dev_dbg(bus->dev,
	...
	if (ret < 0) {
		if (use_multi_lane) {
			...
			dev_warn(bus->dev, "%s: could not find frame configuration for bus dr_freq %d try multi-lane\n",
				__func__, curr_dr_freq);
			goto multilane;
		}
	}
