...
int sdw_compute_params(struct sdw_bus *bus, struct sdw_stream_runtime *stream)
{
	...
	/*
	 * stream->state is set to SDW_STREAM_DEPREPARED at the beginning of
	 * _sdw_deprepare_stream(). In other words, if stream->state != SDW_STREAM_DEPREPARED,
	 * sdw_compute_params() is called from _sdw_prepare_stream() and the stream is preparing.
	 */
