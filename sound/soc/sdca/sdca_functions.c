...
int sdca_get_mic_count(struct sdw_slave *slave, struct sdca_function_desc *function)
{
	...
	for (i = 0; i < num_entities; i++) {
		...
		if (entity_type == SDCA_ENTITY_TYPE_IT) {
			...
			if (terminal_type != 0x201 && terminal_type != 0x205)
				continue;
			...
			dev_info(&slave->dev,
				 "SDCA function %s entity %s: iot mipi-sdca-terminal-type=%s0x%x mipi-sdca-terminal-transducer-count=%s%u\n",
				 function->name, entity_label,
				 terminal_type == U32_MAX ? "missing " : "",
				 terminal_type == U32_MAX ? 0 : terminal_type,
				 transducer_count == U32_MAX ? "missing " : "",
				 transducer_count == U32_MAX ? 0 : transducer_count);
			break;
		}
