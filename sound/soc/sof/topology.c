...
static const struct sof_topology_token pin_tokens[] = {
			    ...
			    struct snd_soc_tplg_dapm_widget *tw)
{
	...
	if (ret != 0) {
		dev_err(scomp->dev, "error: parse num pins tokens failed %d %d\n",
			swidget->num_source_pins, swidget->num_sink_pins);
		return -EINVAL;
	}

	dev_dbg(scomp->dev, "tplg: ready widget id %d pipe %d type %d name : %s stream %s num_source_pins %d num_sink_pins %d\n",
