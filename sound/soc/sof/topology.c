...
int snd_sof_load_topology(struct snd_soc_component *scomp, const char *file)
{
	...
	tplg_name = kasprintf(GFP_KERNEL, "%s", file);
	...
	for (i = 0; i < tplg_num; i++) {
		...
		case TPLG_DEVICE_PCH_DMIC:
			tplg_files[i].file = kasprintf(GFP_KERNEL, "%s/sof-%s-%s-id%d.tplg",
						       sof_pdata->tplg_filename_prefix,
						       platform, tplg_files[i].device,
						       tplg_files[i].be_id);
			break;
		default:
			tplg_files[i].file = kasprintf(GFP_KERNEL, "%s/sof-%s-id%d.tplg",
						       sof_pdata->tplg_filename_prefix,
