	...
	for_each_pcm_streams(dir) {
		...
		if (w) {
			...
			if (dir == SNDRV_PCM_STREAM_PLAYBACK) {
				if (ipc4_copier->available_fmt.num_output_formats == 1) {
					ipc4_fmt = &ipc4_copier->available_fmt.
						   output_pin_fmts->audio_fmt;
					single_fmt = true;
				}
			} else {
				if (ipc4_copier->available_fmt.num_input_formats == 1) {
					ipc4_fmt = &ipc4_copier->available_fmt.
						   input_pin_fmts->audio_fmt;
					single_fmt = true;
				}
			}
