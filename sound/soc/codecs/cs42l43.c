...
static int cs42l43_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	int provider = !!regmap_test_bits(priv->regmap, CS42L43_ASP_CLK_CONFIG2,
				   ...
				   struct snd_soc_dai *dai)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_SAMPLE_RATE1,
...
{
	struct cs42l43_codec *priv = snd_soc_component_get_drvdata(dai->component);
	int dsp_mode = !!regmap_test_bits(priv->regmap, CS42L43_ASP_CTRL,
	...
	if (provider) {
		...
		regmap_update_bits(priv->regmap, CS42L43_ASP_CLK_CONFIG1,
				   CS42L43_ASP_BCLK_N_MASK | CS42L43_ASP_BCLK_M_MASK,
				   n << CS42L43_ASP_BCLK_N_SHIFT |
				   m << CS42L43_ASP_BCLK_M_SHIFT);
		regmap_update_bits(priv->regmap, CS42L43_ASP_FSYNC_CTRL1,
				   CS42L43_ASP_FSYNC_M_MASK, frame);
	}

	regmap_update_bits(priv->regmap, CS42L43_ASP_FSYNC_CTRL4,
...
static int cs42l43_asp_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_ASP_CTRL,
			   CS42L43_ASP_FSYNC_MODE_MASK,
			   asp_ctrl);
	regmap_update_bits(priv->regmap, CS42L43_ASP_DATA_CTRL,
			   CS42L43_ASP_FSYNC_FRAME_START_DLY_MASK |
			   CS42L43_ASP_FSYNC_FRAME_START_PHASE_MASK,
			   data_ctrl);
	regmap_update_bits(priv->regmap, CS42L43_ASP_CLK_CONFIG2,
			   CS42L43_ASP_MASTER_MODE_MASK |
			   CS42L43_ASP_BCLK_INV_MASK,
			   clk_config);
	regmap_update_bits(priv->regmap, CS42L43_ASP_FSYNC_CTRL3,
				 ...
				 struct snd_soc_dai *dai)
{
	...
	ret = cs42l43_sdw_add_slave(substream, params, dai);
	if (ret)
		return ret;
...
static const struct snd_soc_dai_ops cs42l43_sdw_ops = {
	...
	.hw_free	= cs42l43_sdw_remove_slave,
};
...
static void cs42l43_spk_vu_sync(struct cs42l43_codec *priv)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_INTP_VOLUME_CTRL1,
...
static int cs42l43_shutter_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs42l43_codec *priv = snd_soc_component_get_drvdata(component);
	 ...
	 * we force a read.
	 */
	ret = regcache_drop_region(priv->regmap, mc->reg, mc->reg);
	if (ret) {
		dev_err(priv->dev, "Failed to drop shutter from cache: %d\n", ret);
		goto error;
	}

	ret = regmap_read(priv->regmap, mc->reg, &val);
	if (ret) {
		dev_err(priv->dev, "Failed to check shutter status: %d\n", ret);
		goto error;
	}

	ucontrol->value.integer.value[0] = !!(val & BIT(mc->shift));

	dev_dbg(priv->dev, "%s shutter is %s\n",
		BIT(mc->shift) == CS42L43_STATUS_MIC_SHUTTER_MUTE_MASK ? "Mic" : "Speaker",
		ucontrol->value.integer.value[0] ? "closed" : "open");
...
static const struct snd_kcontrol_new cs42l43_controls[] = {
	...
	SOC_DOUBLE_R_EXT("Speaker Digital Switch",
			 ...
			 snd_soc_get_volsw, cs42l43_spk_put),
	...
	SOC_SINGLE_EXT("Microphone Shutter Switch", CS42L43_SHUTTER_CONTROL,
		       CS42L43_STATUS_MIC_SHUTTER_MUTE_SHIFT, 1, 1,
		       cs42l43_shutter_get, NULL),
	SOC_SINGLE_EXT("Speaker Shutter Switch", CS42L43_SHUTTER_CONTROL,
		       CS42L43_STATUS_SPK_SHUTTER_MUTE_SHIFT, 1, 1,
		       cs42l43_shutter_get, NULL),
};
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		regmap_update_bits(priv->regmap, CS42L43_MUTE_EQ_IN0,
				   CS42L43_MUTE_EQ_CH1_MASK | CS42L43_MUTE_EQ_CH2_MASK,
				   CS42L43_MUTE_EQ_CH1_MASK | CS42L43_MUTE_EQ_CH2_MASK);

		regmap_update_bits(priv->regmap, CS42L43_COEFF_RD_WR0,
				   CS42L43_WRITE_MODE_MASK, CS42L43_WRITE_MODE_MASK);

		for (i = 0; i < CS42L43_N_EQ_COEFFS; i++)
			regmap_write(priv->regmap, CS42L43_COEFF_DATA_IN0,
				     priv->eq_coeffs[i]);
	...
	case SND_SOC_DAPM_POST_PMU:
		ret = regmap_read_poll_timeout(priv->regmap, CS42L43_INIT_DONE0,
...
static int cs42l43_calc_pll(struct cs42l43_codec *priv, unsigned int src,
			    unsigned int freq)
{
	dev_dbg(priv->dev, "Source PLL from %s at %uHz\n",
		src ? "SoundWire" : "MCLK", freq);

	lockdep_assert_held(&priv->pll_lock);

	if (priv->pll_config && priv->refclk_src == src &&
	    (priv->pll_config->freq << priv->refclk_div) == freq)
		return 0;

	if (regmap_test_bits(priv->regmap, CS42L43_CTRL_REG, CS42L43_PLL_EN_MASK)) {
		dev_err(priv->dev, "PLL active, can't change configuration\n");
		return -EBUSY;
	}

	priv->pll_config = NULL;
	...
	if (priv->refclk_div <= CS42L43_PLL_REFCLK_DIV_MASK) {
		int i;

		for (i = 0; i < ARRAY_SIZE(cs42l43_pll_configs); i++) {
			if (freq == cs42l43_pll_configs[i].freq) {
				priv->pll_config = &cs42l43_pll_configs[i];
				return 0;
			}
		}
	}

	dev_err(priv->dev, "No suitable PLL config: %uHz, 0x%x\n",
		freq, priv->refclk_div);
	return -EINVAL;
}

static int cs42l43_enable_pll(struct cs42l43_codec *priv)
{
	const struct cs42l43_pll_config *config = priv->pll_config;
	...
	dev_dbg(priv->dev, "Enabling PLL\n");

	lockdep_assert_held(&priv->pll_lock);

	if (!config) {
		dev_err(priv->dev, "No valid PLL configuration\n");
		return -ENOENT;
	}

	regmap_update_bits(priv->regmap, CS42L43_PLL_CONTROL,
			   CS42L43_PLL_REFCLK_DIV_MASK | CS42L43_PLL_REFCLK_SRC_MASK,
			   priv->refclk_div << CS42L43_PLL_REFCLK_DIV_SHIFT |
			   priv->refclk_src << CS42L43_PLL_REFCLK_SRC_SHIFT);
	regmap_write(priv->regmap, CS42L43_FDIV_FRAC, config->div);
	regmap_update_bits(priv->regmap, CS42L43_CTRL_REG,
			   CS42L43_PLL_MODE_BYPASS_500_MASK |
			   CS42L43_PLL_MODE_BYPASS_1029_MASK,
			   config->mode << CS42L43_PLL_MODE_BYPASS_1029_SHIFT);
	regmap_update_bits(priv->regmap, CS42L43_CAL_RATIO,
	 ...
	 */
	regmap_multi_reg_write(priv->regmap, enable_seq, ARRAY_SIZE(enable_seq));
...
static int cs42l43_disable_pll(struct cs42l43_codec *priv)
{
	...
	regmap_multi_reg_write(priv->regmap, disable_seq, ARRAY_SIZE(disable_seq));
	regmap_update_bits(priv->regmap, CS42L43_CTRL_REG, CS42L43_PLL_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_PLL_CONTROL, CS42L43_PLL_REFCLK_EN_MASK, 0);
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (!priv->load_detect_running)
			regmap_update_bits(priv->regmap, CS42L43_BLOCK_EN8, mask, val);
		break;
	case SND_SOC_DAPM_POST_PMU:
	case SND_SOC_DAPM_POST_PMD:
		if (priv->load_detect_running)
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(priv->regmap, reg, mute | ramp, mute);
		break;
	case SND_SOC_DAPM_POST_PMU:
		regmap_update_bits(priv->regmap, reg, mute | ramp, *val);
		break;
	default:
		break;
	}
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (!priv->load_detect_running)
			regmap_update_bits(priv->regmap, CS42L43_BLOCK_EN3, mask, val);
		fallthrough;
	default:
		return 0;
	}
			      ...
			      int src, unsigned int freq, int dir)
{
	...
	mutex_lock(&priv->pll_lock);
	ret = cs42l43_calc_pll(priv, src, freq);
	mutex_unlock(&priv->pll_lock);
