...
//
// rt1316-sdw.c -- rt1316 ALSA SoC audio driver
//
...
static bool rt1316_readable_register(struct device *dev, unsigned int reg)
{
	...
	case 0xcf00 ... 0xcf03:
	case 0xd101 ... 0xd103:
	case RT1316_SDCA_ADDR(0x4, 0x2, 0x10, 0x0):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x1, 0x1):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x1, 0x2):
	case RT1316_SDCA_ADDR(0x4, 0x31, 0x1, 0x0):
	case RT1316_SDCA_ADDR(0x4, 0x33, 0x1, 0x0):
	case RT1316_SDCA_ADDR(0x4, 0x34, 0x1, 0x0):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x2, 0x1) | BIT(13):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x2, 0x1):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x2, 0x2) | BIT(13):
	case RT1316_SDCA_ADDR(0x4, 0x3, 0x2, 0x2):
		return true;
	default:
		return false;
	}
...
/* Bus clock frequency */
#define RT1316_CLK_FREQ_9600000HZ 9600000
#define RT1316_CLK_FREQ_12000000HZ 12000000
#define RT1316_CLK_FREQ_6000000HZ 6000000
#define RT1316_CLK_FREQ_4800000HZ 4800000
#define RT1316_CLK_FREQ_2400000HZ 2400000

static int rt1316_clock_config(struct device *dev)
{
	struct rt1316_sdw_priv *rt1316 = dev_get_drvdata(dev);
	unsigned int clk_freq, base_value, scale_value;

	clk_freq = (rt1316->params.curr_dr_freq >> 1);

	switch (clk_freq) {
	case RT1316_CLK_FREQ_12000000HZ:
		base_value = 0x2;
		scale_value = 0x2;
		break;
	case RT1316_CLK_FREQ_6000000HZ:
		base_value = 0x2;
		scale_value = 0x3;
		break;
	case RT1316_CLK_FREQ_9600000HZ:
		base_value = 0x1;
		scale_value = 0x2;
		break;
	case RT1316_CLK_FREQ_4800000HZ:
		base_value = 0x1;
		scale_value = 0x3;
		break;
	case RT1316_CLK_FREQ_2400000HZ:
		base_value = 0x1;
		scale_value = 0x4;
		break;
	default:
	...
	regmap_write(rt1316->regmap, 0x4d, base_value);
	regmap_write(rt1316->regmap, 0x62, scale_value);
	regmap_write(rt1316->regmap, 0x72, scale_value);
...
static int rt1316_io_init(struct device *dev, struct sdw_slave *slave)
{
	...
	//PLL_in=SdW_CLK, Sys=FOUT_PLL2_SDW=12.288
	...
	//fs1, clk_da=div1, clk_sil_det=div1
	regmap_write(rt1316->regmap, 0xc006, 0x00);
	//fs2, clk_da=div1, clk_sil_det=div1
	...
	regmap_write(rt1316->regmap,
		RT1316_SDCA_ADDR(0x4, 0x24, 0x1, 0x0), 0x00);
				...
				struct sdw_bus_params *params)
{
	...
	memcpy(&rt1316->params, params, sizeof(*params));

	ret = rt1316_clock_config(&slave->dev);
	if (ret < 0)
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x31, 0x1, 0x0), 0x0);
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x32, 0x1, 0x0), 0x0);
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x33, 0x1, 0x0), 0x0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x31, 0x1, 0x0), 0x3);
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x32, 0x1, 0x0), 0x3);
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x33, 0x1, 0x0), 0x3);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x34, 0x1, 0x0), 0x0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_write(component,
			RT1316_SDCA_ADDR(0x4, 0x34, 0x1, 0x0), 0x3);
		break;
	}
	return 0;
}
...
static SOC_ENUM_SINGLE_DECL(rt1316_rx_data_ch_enum,
	RT1316_SDCA_ADDR(0x4, 0x2, 0x10, 0x0), 0,
...
static SOC_ENUM_SINGLE_DECL(rt1316_xu24_bypass_enum,
	RT1316_SDCA_ADDR(0x4, 0x24, 0x1, 0x0), 0,
...
static const struct snd_kcontrol_new rt1316_sto_dac_l =
	SOC_DAPM_SINGLE_AUTODISABLE("Switch",
		RT1316_SDCA_ADDR(0x4, 0x3, 0x1, 0x1),
		0, 1, 1);

static const struct snd_kcontrol_new rt1316_sto_dac_r =
	SOC_DAPM_SINGLE_AUTODISABLE("Switch",
		RT1316_SDCA_ADDR(0x4, 0x3, 0x1, 0x2),
		0, 1, 1);
static const struct snd_kcontrol_new rt1316_i_control =
	SOC_DAPM_DOUBLE("Switch", 0xc605, 2, 0, 1, 1);

static const struct snd_kcontrol_new rt1316_v_control =
	SOC_DAPM_DOUBLE("Switch", 0xc605, 3, 1, 1, 1);
...
static const struct snd_soc_dapm_widget rt1316_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("PDE24", SND_SOC_NOPM, 0, 0,
		rt1316_pde24_event,
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SWITCH("I Sense", SND_SOC_NOPM, 0, 0,
		&rt1316_i_control),
	SND_SOC_DAPM_SWITCH("V Sense", SND_SOC_NOPM, 0, 0,
		&rt1316_v_control),
...
static const struct snd_soc_dapm_route rt1316_dapm_routes[] = {
	...
	{ "I Sense", "Switch", "I Gen" },
	{ "V Sense", "Switch", "V Gen" },
	{ "I Sense", NULL, "PDE24" },
	{ "V Sense", NULL, "PDE24" },
	{ "DP2TX", NULL, "I Sense" },
	{ "DP2TX", NULL, "V Sense" },
...
#define RT1316_FORMATS (SNDRV_PCM_FMTBIT_S8 | \
...
regmap_sync:
	...
	regcache_cache_only(rt1316->regmap, false);
	regcache_sync_region(rt1316->regmap, 0xc000, 0xd103);
	regcache_sync_region(rt1316->regmap, 0x41000000, 0x41080100);
...
MODULE_DESCRIPTION("ASoC RT1316 driver SDW");
