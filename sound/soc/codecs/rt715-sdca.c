...
{
	struct regmap *regmap = rt715->regmap;
	unsigned int addr;
	int ret;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mc->shift == 8)
		...
		gain_l_val = abs(gain_l_val - mc->shift) * 375;
		gain_l_val <<= 8;
		gain_l_val /= 1000;
		if (val_l <= mc->shift) {
	...
	if (mc->shift == 8)
		...
		gain_r_val = abs(gain_r_val - mc->shift) * 375;
		gain_r_val <<= 8;
		gain_r_val /= 1000;
		if (val_r <= mc->shift) {
	...
	for (i = 0; i < 3; i++) { /* retry 3 times at most */
		...
		regmap_read(rt715->regmap, mc->reg, &val_l);
		regmap_read(rt715->regmap, mc->rreg, &val_r);
				...
				struct snd_soc_dai *dai)
{
	...
	regmap_write(rt715->regmap,
		RT715_SDCA_ADDR(FUN_MIC_ARRAY, RT715_SDCA_CS_FREQ_IND_EN,
			RT715_SDCA_FREQ_IND_CTRL, CH_00),
			val);
