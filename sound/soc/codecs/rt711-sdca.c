...
static void rt711_sdca_reset(struct rt711_sdca_priv *rt711)
{
	...
	rt711_sdca_index_update_bits(rt711, RT711_VENDOR_CALI,
		RT711_DAC_DC_CALI_CTL1, 0x8, 0x0);
	rt711_sdca_index_update_bits(rt711, RT711_VENDOR_CALI,
		RT711_DAC_DC_CALI_CTL1, 0x8, 0x8);
}
...
static int rt711_sdca_calibration(struct rt711_sdca_priv *rt711)
{
	...
	for (loop = 0; loop < 500 && !(val & 0x40); loop++) {
	...
	for (loop = 0; loop < 500 &&
		(val & RT711_DAC_DC_CALI_TRIGGER); loop++) {
		usleep_range(10000, 11000);
...
static void rt711_sdca_jack_detect_handler(struct work_struct *work)
{
	...
	int btn_type = 0, ret;
	unsigned int sdca_stat0, sdca_stat8;
	...
	ret = regmap_read(rt711->regmap, SDW_SCP_SDCA_INT1, &sdca_stat0);
	if (ret < 0)
		goto io_error;
	if (sdca_stat0) {
	...
	if (sdca_stat0 & SDW_SCP_SDCA_INT_SDCA_0) {
	...
	ret = regmap_read(rt711->regmap, SDW_SCP_SDCA_INT2, &sdca_stat8);
	if (ret < 0)
		goto io_error;
	if (sdca_stat8) {
	...
	if ((rt711->jack_type == SND_JACK_HEADSET) &&
		(sdca_stat8 & SDW_SCP_SDCA_INT_SDCA_8))
	...
	dev_dbg(&rt711->slave->dev,
		"in %s, btn_type=0x%x\n", __func__, btn_type);
	dev_dbg(&rt711->slave->dev,
		"in %s, sdca_stat0=0x%x, sdca_stat8=0x%x\n", __func__, sdca_stat0, sdca_stat8);
...
static void rt711_sdca_jack_init(struct rt711_sdca_priv *rt711)
{
	...
	if (rt711->hs_jack) {
		...
		case RT711_JD2:
			rt711_sdca_index_update_bits(rt711, RT711_VENDOR_REG,
				RT711_JD_CTL2, RT711_HP_JD_SEL_JD2,
				RT711_HP_JD_SEL_JD2);
			rt711_sdca_index_update_bits(rt711, RT711_VENDOR_REG,
				RT711_CC_DET1,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < 3; i++) { /* retry 3 times at most */
		/* Lch*/
		regmap_write(rt711->regmap, mc->reg, gain_l_val);

		/* Rch */
		regmap_write(rt711->regmap, mc->rreg, gain_r_val);

		regmap_read(rt711->regmap, mc->reg, &read_l);
		regmap_read(rt711->regmap, mc->rreg, &read_r);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = rt711_sdca_index_read(rt711, RT711_VENDOR_HDA_CTL,
...
int rt711_sdca_init(struct device *dev, struct regmap *sdw_regmap,
