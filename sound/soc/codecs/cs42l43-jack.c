		     ...
		     struct snd_soc_jack *jack, void *d)
{
	...
	if (pdata->bias_sense_ua) {
		...
		autocontrol |= 0x3 << CS42L43_HSBIAS_SENSE_TRIP_SHIFT;
	}
	...
	if (pdata->use_ring_sense) {
		...
		regmap_update_bits(priv->regmap, CS42L43_RINGSENSE_DEB_CTRL,
	...
	regmap_update_bits(priv->regmap, CS42L43_HS_BIAS_SENSE_AND_CLAMP_AUTOCONTROL,
			   CS42L43_JACKDET_MODE_MASK | CS42L43_S0_AUTO_ADCMUTE_DISABLE_MASK |
			   CS42L43_HSBIAS_SENSE_TRIP_MASK, autocontrol);
	regmap_update_bits(priv->regmap, CS42L43_PDNCNTL,
...
static void cs42l43_start_hs_bias(struct cs42l43_codec *priv, bool force_high)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_MIC_DETECT_CONTROL_1,
...
static inline bool cs42l43_jack_present(struct cs42l43_codec *priv)
{
	...
	regmap_read(priv->regmap, CS42L43_TIP_RING_SENSE_INTERRUPT_STATUS, &sts);
...
irqreturn_t cs42l43_button_press(int irq, void *data)
{
	...
	if (!priv->button_detect_running) {
		dev_warn(priv->dev, "Spurious button press IRQ\n");
		goto error;
	}
	...
	regmap_read(priv->regmap, CS42L43_DETECT_STATUS_1, &val);

	dev_dbg(priv->dev, "Button press IRQ: 0x%x\n", val);
	...
	if (!buttons)
		dev_warn(priv->dev, "Unrecognised button: %d ohms\n", val);
...
irqreturn_t cs42l43_button_release(int irq, void *data)
{
	...
	if (!priv->button_detect_running) {
		dev_warn(priv->dev, "Spurious button release IRQ\n");
		mutex_unlock(&priv->jack_lock);
...
static void cs42l43_start_load_detect(struct cs42l43_codec *priv)
{
	...
	if (priv->hp_ena) {
		...
		regmap_update_bits(priv->regmap, CS42L43_BLOCK_EN8,
	...
	regmap_update_bits(priv->regmap, CS42L43_BLOCK_EN3,
			   CS42L43_ADC1_EN_MASK | CS42L43_ADC2_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_DACCNFG2, CS42L43_HP_HPF_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_MIC_DETECT_CONTROL_1,
			   CS42L43_HSBIAS_MODE_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_CTRL,
			   CS42L43_ADPTPWR_MODE_MASK, 0x4 << CS42L43_ADPTPWR_MODE_SHIFT);
	regmap_update_bits(priv->regmap, CS42L43_PGAVOL,
			   CS42L43_HP_DIG_VOL_RAMP_MASK | CS42L43_HP_ANA_VOL_RAMP_MASK, 0x6);
	regmap_update_bits(priv->regmap, CS42L43_DACCNFG1,
			   CS42L43_HP_MSTR_VOL_CTRL_EN_MASK, 0);

	regmap_update_bits(priv->regmap, CS42L43_HS2,
			   CS42L43_HS_CLAMP_DISABLE_MASK, CS42L43_HS_CLAMP_DISABLE_MASK);

	regmap_update_bits(priv->regmap, CS42L43_LOADDETENA,
...
static void cs42l43_stop_load_detect(struct cs42l43_codec *priv)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_LOADDETENA, CS42L43_HPLOAD_DET_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_HS2, CS42L43_HS_CLAMP_DISABLE_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_DACCNFG1,
			   CS42L43_HP_MSTR_VOL_CTRL_EN_MASK, CS42L43_HP_MSTR_VOL_CTRL_EN_MASK);
	regmap_update_bits(priv->regmap, CS42L43_PGAVOL,
	...
	regmap_update_bits(priv->regmap, CS42L43_MIC_DETECT_CONTROL_1,
			   CS42L43_HSBIAS_MODE_MASK, 0x1 << CS42L43_HSBIAS_MODE_SHIFT);
	regmap_update_bits(priv->regmap, CS42L43_DACCNFG2,
...
static int cs42l43_run_load_detect(struct cs42l43_codec *priv, bool mic)
{
	...
	regmap_read(priv->regmap, CS42L43_LOADDETRESULTS, &val);
...
static int cs42l43_run_type_detect(struct cs42l43_codec *priv)
{
	...
	regmap_read(priv->regmap, CS42L43_HS_STAT, &type);
...
static void cs42l43_clear_jack(struct cs42l43_codec *priv)
{
	...
	regmap_update_bits(priv->regmap, CS42L43_ADC_B_CTRL1,
			   CS42L43_PGA_WIDESWING_MODE_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_ADC_B_CTRL2,
			   CS42L43_PGA_WIDESWING_MODE_EN_MASK, 0);
	regmap_update_bits(priv->regmap, CS42L43_STEREO_MIC_CTRL,
...
int cs42l43_jack_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!override) {
		...
		regmap_update_bits(priv->regmap, CS42L43_STEREO_MIC_CLAMP_CTRL,
