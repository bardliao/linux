				...
				int *sdw_be_num, int *sdw_cpu_dai_num)
{
	...
	int _dai_type = SOF_SDW_DAI_TYPE_JACK;
			...
			for (j = 0; j < codec_info->dai_num; j++) {
				...
				for_each_pcm_streams(stream) {
					if (!codec_info->dais[j].direction[stream])
						continue;
					pr_err("bard: %s %s dir %d\n",
						__func__, codec_info->dais[j].dai_name, stream);
				     ...
				     struct snd_soc_card *card)
{
	...
	for (; adr_link->num_adr; adr_link++) {
		...
		for (i = 0; i < adr_link->num_adr; i++) {
			/* find codec info to create BE DAI */
			codec_index = find_codec_info_part(adr_link->adr_d[i].adr);
			pr_err("bard: %s %#llx codec_index %d .dai_num %d\n", __func__, adr_link->adr_d[i].adr, codec_index, codec_info_list[codec_index].dai_num);
			if (codec_index < 0)
				return codec_index;
...
out:
	...
	for (adr_link = mach_params->links ; adr_link->num_adr; adr_link++) {
		for (i = 0; i < adr_link->num_adr; i++) {
			...
			/* find codec info to create BE DAI */
			codec_index = find_codec_info_part(adr_link->adr_d[i].adr);
			pr_err("bard: %s codec_index %d\n", __func__, codec_index);
			if (codec_index < 0)
				return codec_index;
...
HDMI:
	...
	for (i = 0; i < card->num_configs; i++) {
               pr_err("bard: i:%d %s: %s\n", i, card->codec_conf[i].dlc.name, card->codec_conf[i].name_prefix);
       }
...
static void mc_dailink_exit_loop(struct snd_soc_card *card)
{
	...
	for (i = 0; i < ARRAY_SIZE(codec_info_list); i++) {
		...
		for_each_card_prelinks(card, j, link) {
			for (k = 0; k < codec_info_list[i].dai_num; k++) {
				if (strcmp(link->codecs[0].dai_name,
				    codec_info_list[i].dais[k].dai_name))
					continue;
