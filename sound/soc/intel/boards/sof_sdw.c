...
static struct sof_sdw_codec_info codec_info_list[] = {
	...
	},
	{
		.part_id = 0x712,
		.version_id = 3,
		.direction = {{true, true}, {true, false}},
		.dai_name = {"rt712-sdca-aif1", "rt712-sdca-aif2"},
		.dai_num = 2,
//		.init = sof_sdw_rt711_sdca_init,
//		.exit = sof_sdw_rt711_sdca_exit,
...
//		.init = sof_sdw_rt711_sdca_init,
//		.exit = sof_sdw_rt711_sdca_exit,
		.codec_dai_type = {SOF_SDW_CODEC_TYPE_MIC},
	},
	{
		.part_id = 0x711,
		.version_id = 2,
	...
	},
	{
		...
		.codec_dai_type = {SOF_SDW_CODEC_TYPE_AMP},
	},
	{
		.part_id = 0x1316,
		.direction = {{true, true}},
				...
				int *sdw_be_num, int *sdw_cpu_dai_num)
{
	...
	int _codec_dai_type = SOF_SDW_CODEC_TYPE_JACK;
	...
	*sdw_be_num  = 0;
	...
	for (link = links; link->num_adr; link++) {
		...
		if (codec_info_list[codec_index].codec_dai_type[0] < _codec_dai_type)
			dev_warn(dev,
				 "Unexpected address table ordering. Expected order: jack -> amp -> mic\n");

		/* _codec_dai_type = the last codec_dai_type of the codec */
		_codec_dai_type = codec_info_list[codec_index].codec_dai_type[codec_info_list[codec_index].dai_num - 1];
		...
		for (i = 0; i < codec_info_list[codec_index].dai_num; i++) {
			...
			for_each_pcm_streams(stream) {
				if (!codec_info_list[codec_index].direction[i][stream])
					continue;
			      ...
			      bool *ignore_pch_dmic)
{
	...
	codecs = devm_kcalloc(dev, codec_num * codec_info_list[codec_index].dai_num,
	...
	for (dai_index = 0; dai_index < codec_info_list[codec_index].dai_num; dai_index++) {
		...
		if (codec_info_list[codec_index].codec_dai_type[dai_index] ==
		    SOF_SDW_CODEC_TYPE_AMP && *link_id < SDW_AMP_DAI_ID)
		 ...
		 */
		if (codec_info_list[codec_index].codec_dai_type[dai_index] ==
		    SOF_SDW_CODEC_TYPE_MIC && *link_id < SDW_DMIC_DAI_ID)
		...
		for_each_pcm_streams(stream) {
			...
			if (!codec_info_list[codec_index].direction[dai_index][stream])
				continue;
			...
			for (index = 0; index < *link_index; index++) {
				/*
				 * Append codec dai type to dai link name if the dai link name is
				 * duplicated.
				 */
				if (!strcmp(name, dai_links[index].name)) {
					...
					name = devm_kasprintf(dev, GFP_KERNEL, "%s-%s",
							      ...
							      type_strings[codec_info_list[codec_index].codec_dai_type[dai_index]]);
				}
			}
			/*
			...
			init_dai_link(dev, dai_links + *link_index, (*link_id)++, name,
				      ...
				      (codecs + dai_index * codec_num), codec_num,
				      NULL, &sdw_ops);
...
SSP:
	...
	for (i = 0, j = 0; ssp_mask; i++, ssp_mask >>= 1) {
		...
		/* TODO: support multi codec dai on SSP when it is needed */
		playback = info->direction[0][SNDRV_PCM_STREAM_PLAYBACK];
		capture = info->direction[0][SNDRV_PCM_STREAM_CAPTURE];
